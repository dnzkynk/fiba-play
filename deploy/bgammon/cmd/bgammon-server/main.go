...
func main() {
	...
	// FIBA yaması: sertifikasız WS'e izin ver (yerel test / TLS'i reverse proxy üstlenir)
	if wsAddress != "" && (op.CertFile == "" || op.CertKey == "") {
		log.Println("Warning: WebSocket listener running without TLS (local/proxy mode).")
	}
