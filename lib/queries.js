...
export async function tournamentWithMatches(id) {
  ...
  const matches = await q(
    `SELECT m.*, p1.full_name AS p1_name, p1.country AS p1_country, p1.company AS p1_company,
            p2.full_name AS p2_name, p2.country AS p2_country, p2.company AS p2_company, w.full_name AS winner_name
     FROM matches m
