  ...
  for (const m of rows) {
    ...
    for (const [name, email, opp] of [[m.n1, m.e1, m.n2], [m.n2, m.e2, m.n1]]) {
      const { subject, html } = matchScheduledMail({
        name: name.split(" ")[0],
        opponent: opp,
        roundLabel: roundName(m.round, total, "tr"),
        timeText: fmt(m.scheduled_at, "tr-TR", "Europe/Istanbul") + " (TR)",
        timeTextEn: fmt(m.scheduled_at, "en-GB", "Europe/Istanbul") + " (Istanbul time)",
      });
