...
async function finishChessMatch(matchId, game) {
  ...
  const status = game.status?.name ?? game.status;
  ...
  if (!game.winner) {
    // Beraberlik: renkler değişir, otomatik rövanş — üst üste MAX_REMATCHES'e kadar
    if (m.rematch_count < MAX_REMATCHES) {
      await resetMatch(matchId, { swapSeats: true, countRematch: true });
  ...
  const winnerId = game.winner === "white" ? m.p1_id : m.p2_id;
  await recordResult(matchId, winnerId, "auto", status ?? "finished");
}
...
function watchMatch(match) {
  ...
  (async () => {
    try {
      ...
      const status = last?.status?.name ?? last?.status;
      if (status && isFinished(status)) {
        await finishChessMatch(match.id, { status, winner: last.winner ?? null });
        return;
      }
...
async function tick() {
  ...
  try {
    ...
    // No-show: 1 saatlik pencere doldu, oyun hâlâ başlamadıysa gelen taraf hükmen kazanır.
    const expired = await q(
      `SELECT m.*, t.game AS tgame FROM matches m
       JOIN tournaments t ON t.id = m.tournament_id
       WHERE m.status = 'live'
         AND COALESCE(m.scheduled_at, m.updated_at) + interval '1 hour' <= now()`
    );
...
export function ensureWatcher() {
  ...
  console.log("⏱  turnuva izleyicisi başladı (60 sn tick)");
}
