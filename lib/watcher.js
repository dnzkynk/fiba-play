...
async function tick() {
  ...
  try {
    ...
    for (const m of live) {
      // Tek maçın hatası (örn. üst tur dolu olduğu için sonuç işlenemiyor) tüm tick'i
      // düşürmemeli: koşul kalıcıysa watcher her 5 sn yeniden çöker ve turnuva durur.
      try {
        const game = await fetchGameStatus(m.game_id).catch(() => null);
        if (game && isFinished(game.status)) {
          await finishChessMatch(m.id, game);
        ...
        console.error(`maç ${m.id} izleme hatası:`, e.message);
      }
    }

    // No-show: hükmen penceresi (ayarlardan, şartname gereği 10 dk) doldu,
    // oyun hâlâ başlamadıysa gelen taraf hükmen kazanır. Pencere, maçın CANLIYA
    // GEÇTİĞİ ana (updated_at) demirlenir; randevu saatine değil. Randevusu geçmiş
    // bir maç yeniden başlatıldığında (iptal edilip tekrar kurulduğunda) oyunculara
    // yine tam süre tanınır — randevuya demirlense pencere çoktan dolmuş sayılır ve
    // maç canlıya geçer geçmez anında hükmen iptal olurdu.
    // updated_at yalnız başlatma/sıfırlamada değişir; oyuncu katılımı onu ötelemez.
    ...
    const expired = await q(
      `SELECT m.*, t.game AS tgame FROM matches m
       JOIN tournaments t ON t.id = m.tournament_id
       WHERE m.status = 'live'
         AND m.updated_at + make_interval(mins => $1) <= now()`,
      [noShowMin]
    );
    for (const m of expired) {
      try {
        ...
        console.error(`maç ${m.id} hükmen işlenemedi:`, e.message);
      }
    }
    ...
    for (const m of orphans) {
      try {
        ...
        console.error(`maç ${m.id} iptal yayılımı hatası:`, e.message);
      }
    }
  } catch (err) {
