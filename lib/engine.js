...
// Boş turnuva oluştur (kura sonra çekilir). round_times: her tur için ISO saat dizisi (opsiyonel).
export async function createTournament({ name, bracketSize, game = "chess", roundTimes = null }, actor = "otomatik") {
  const size = parseInt(bracketSize, 10);
  if (![8, 16, 32, 64].includes(size)) throw new Error("Turnuva boyu 8/16/32/64 olmalı");
  if (!name?.trim()) throw new Error("Turnuva adı gerekli");
  let times = null;
  if (Array.isArray(roundTimes) && roundTimes.length) {
    const total = Math.log2(size);
    if (roundTimes.length !== total) throw new Error(`${total} tur için ${total} saat girilmeli`);
    times = roundTimes.map((v) => {
      const d = new Date(v);
      if (isNaN(d)) throw new Error("Geçersiz tarih");
  ...
  const [t] = await q(
    `INSERT INTO tournaments (name, game, bracket_size, status, starts_at, round_times)
     VALUES ($1, $2, $3, 'draft', $4, $5) RETURNING *`,
    [name.trim(), game, size, times?.[0] ?? null, times ? JSON.stringify(times) : null]
  );
  await audit("tournament_create", { id: t.id, name: t.name, size }, actor);
  return t;
}

// Kişiyi turnuvaya ata (kura öncesi üyelik). Aynı turnuvaya iki kez atanamaz.
export async function assignPlayer(tournamentId, participantId, isReserve = false, actor = "otomatik") {
  const [t] = await q("SELECT * FROM tournaments WHERE id = $1", [tournamentId]);
  if (!t) throw new Error("Turnuva bulunamadı");
  if (t.status !== "draft") throw new Error("Kura çekilmiş turnuvaya atama yapılamaz (yedek değişikliğini kullanın)");
  const [p] = await q("SELECT * FROM participants WHERE id = $1", [participantId]);
  if (!p) throw new Error("Katılımcı bulunamadı");
  if (p.game !== t.game) throw new Error("Katılımcı bu turnuvanın oyununa kayıtlı değil");
  const res = await q(
    `INSERT INTO tournament_players (tournament_id, participant_id, is_reserve)
     VALUES ($1, $2, $3) ON CONFLICT (tournament_id, participant_id) DO NOTHING RETURNING participant_id`,
    [tournamentId, participantId, !!isReserve]
  );
  if (!res.length) throw new Error("Bu kişi zaten bu turnuvada");
  await audit("assign_player", { tournamentId, participantId, isReserve: !!isReserve }, actor);
  return { ok: true };
}

// Kişiyi turnuvadan çıkar (yalnızca kura öncesi).
export async function unassignPlayer(tournamentId, participantId, actor = "otomatik") {
  const [t] = await q("SELECT * FROM tournaments WHERE id = $1", [tournamentId]);
  if (!t) throw new Error("Turnuva bulunamadı");
  if (t.status !== "draft") throw new Error("Kura çekilmiş turnuvadan çıkarma yapılamaz");
  await q("DELETE FROM tournament_players WHERE tournament_id = $1 AND participant_id = $2", [tournamentId, participantId]);
  await audit("unassign_player", { tournamentId, participantId }, actor);
  return { ok: true };
}
...
export async function drawTournament(tournamentId, actor = "otomatik") {
  ...
  // Yalnızca bu turnuvaya atanmış asil oyuncular (yedekler kuraya girmez)
  const players = await q(
    `SELECT tp.participant_id AS id FROM tournament_players tp
     WHERE tp.tournament_id = $1 AND NOT tp.is_reserve`,
    [tournamentId]
  );
  if (players.length < 2) throw new Error("Bu turnuva için yeterli katılımcı yok (en az 2 asil oyuncu atayın)");
  ...
  // Turnuvanın kayıtlı programı varsa maç saatlerini turlara göre yaz
  if (Array.isArray(t.round_times) && t.round_times.length === rounds) {
    for (let r = 1; r <= rounds; r++) {
      await q(
        `UPDATE matches SET scheduled_at = $1,
           status = CASE WHEN p1_id IS NOT NULL AND p2_id IS NOT NULL THEN 'scheduled' ELSE status END
         WHERE tournament_id = $2 AND round = $3`,
        [t.round_times[r - 1], tournamentId, r]
      );
    }
  }
